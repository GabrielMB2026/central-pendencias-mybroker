...
function parseCamposERP(raw) {
  ...
  if (vm) {
    statusVenda = 'Venda na aba: ' + vm[1].trim();
    limpo = limpo.slice(0, vm.index).trim();
  }
  const pm = limpo.match(/\(([^)]+)\)\s*$/);
  if (pm) {
    statusVenda = (statusVenda ? statusVenda + ' · ' : '') + pm[1].trim();
    limpo = limpo.slice(0, pm.index).trim();
  }
  ...
  if (ep0 && ep0.nome) {
    result.pagador = ep0.nome;
    result.proposta = ep0.prop;
    result.confianca = 'alta';
  } else if (ep0 && !ep0.nome) {
    result.proposta = ep0.prop;
    result.confianca = 'media';
  } else if (/^proposta$/i.test(b0)) {
  ...
  for (let i = 1; i < blocos.length; i++) {
    ...
    if (epi) {
      if (epi.prop && !result.proposta) result.proposta = epi.prop;
      if (epi.nome && !result.empreendimento) result.empreendimento = epi.nome;
      continue;
    }
    if (/^proposta$/i.test(b)) continue;
    if (!result.empreendimento) {
      result.empreendimento = b;
    } else {
      result.empreendimento += ' · ' + b;
    }
  }

  if (!result.pagador && !result.empreendimento && !result.proposta) {
    result.pagador = blocos.join(' - ');
    result.confianca = 'baixa';
  }
...
export default function Home() {
  ...
  const loadPendencias = useCallback(async () => {
    ...
    const mapped = (data || []).map((row) => ({
      ...
      dataReceb: row.data_receb || '',
      valor: Number(row.valor) || 0,
      tipo: row.tipo || 'Documentação',
  ...
  const filtered = pendencias.filter((p) => {
    ...
    return bOk && (!fStatus || p.status === fStatus);
  });
  ...
  // ── Seleção ──
  const todosSelecioados = filtered.length > 0 && selecionados.length === filtered.length;

  function toggleSelecionarTodos(checked) {
    setSelecionados(checked ? filtered.map((p) => p.id) : []);
  }

  function toggleSelecionado(id, checked) {
    setSelecionados((s) => checked ? [...s, id] : s.filter((x) => x !== id));
  }

  async function deletarSelecionados() {
    if (!confirm(`Excluir ${selecionados.length} pendência(s)? Esta ação não pode ser desfeita.`)) return;
    const { error } = await supabase.from('pendencias').delete().in('id', selecionados);
    if (error) {
    ...
    showToast(`${selecionados.length} pendência(s) removida(s).`);
  }
  ...
  function getMappingByField() {
    ...
    Object.entries(mapping).forEach(([idx, field]) => {
      if (field !== 'ignorar') m[field] = parseInt(idx, 10);
    });
    return m;
  }
  ...
  async function doImport() {
    ...
    importedRows.forEach((row) => {
      ...
      novas.push(novaPendencia);
      seedExisting = [...seedExisting, novaPendencia];
    });
    ...
    showToast(
      novas.length +
        ' pendência(s) importada(s)! ' +
        (baixaMedia ? baixaMedia + ' precisam de revisão manual.' : 'Tudo extraído com alta confiança.')
    );
  }
  ...
  async function savePendencia() {
    ...
    if (editingId) {
      ...
      const payload = {
        ...
        data_receb: form.dataReceb || '',
        valor: parseFloat(form.valor) || 0,
        tipo: form.tipo || 'Documentação',
      ...
      const payload = {
        ...
        acao: form.acao || '',
        obs: form.obs || '',
        historico: [`${dateStr} - Criado por ${resp}`],
        origem: 'manual',
      };
      const { error } = await supabase.from('pendencias').insert(payload);
      setSaving(false);
    ...
    await loadPendencias();
    closeModal();
    showToast('Pendência salva com sucesso!');
  }
  ...
  function exportExcel() {
    ...
    const rows = pendencias.map((p) => ({
      ID: p.id,
      Origem: p.origem === 'erp' ? 'ERP' : 'Manual',
      'C.Custo / Loja': p.loja,
      Pagador: p.pagador,
      Empreendimento: p.empreendimento,
      'Proposta / Contrato': p.proposta,
      'Data Receb.': p.dataReceb,
      'Valor (R$)': p.valor,
      Status: p.status,
    ...
    ws['!cols'] = [
      { wch: 10 }, { wch: 8 }, { wch: 30 }, { wch: 26 }, { wch: 24 }, { wch: 16 },
      { wch: 12 }, { wch: 14 }, { wch: 14 }, { wch: 20 }, { wch: 32 }, { wch: 42 }, { wch: 60 },
    ];
  ...
  const porLoja = Object.values(
    pendencias.reduce((acc, p) => {
      ...
      if (!acc[loja]) acc[loja] = { loja, total: 0, valor: 0, pendente: 0, atrasada: 0, resolvida: 0 };
      acc[loja].total += 1;
      acc[loja].valor += Number(p.valor) || 0;
      if (p.status === 'Pendente' || p.status === 'Em andamento') acc[loja].pendente += 1;
      if (p.status === 'Atrasada') acc[loja].atrasada += 1;
      if (p.status === 'Resolvida') acc[loja].resolvida += 1;
      return acc;
    }, {})
  ...
  const porResponsavel = Object.values(
    pendencias.reduce((acc, p) => {
      const resp = p.responsavel || '(a definir)';
      if (!acc[resp]) acc[resp] = { responsavel: resp, total: 0, valor: 0, pendente: 0, atrasada: 0, resolvida: 0 };
      acc[resp].total += 1;
      acc[resp].valor += Number(p.valor) || 0;
      if (p.status === 'Pendente' || p.status === 'Em andamento') acc[resp].pendente += 1;
      if (p.status === 'Atrasada') acc[resp].atrasada += 1;
      if (p.status === 'Resolvida') acc[resp].resolvida += 1;
      return acc;
    }, {})
  ).sort((a, b) => b.total - a.total);

  const maxValorLoja = porLoja.length ? porLoja[0].valor : 1;
  const maxTotalResp = porResponsavel.length ? porResponsavel[0].total : 1;
  ...
  return (
    <>
      ...
      {/* ── ABAS ── */}
      <div style={{ display: 'flex', gap: 0, borderBottom: '2px solid rgba(255,255,255,.07)', background: 'var(--bg2, #0f0f1a)', padding: '0 24px' }}>
        {[['pendencias', '📋 Pendências'], ['dashboard', '📊 Dashboard']].map(([key, label]) => (
          <button
            key={key}
            onClick={() => setAba(key)}
            style={{
              background: 'none', border: 'none', cursor: 'pointer',
              padding: '12px 20px', fontSize: 12, fontFamily: "'DM Sans', sans-serif",
              fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase',
              color: aba === key ? '#f59e0b' : 'rgba(255,255,255,.4)',
              borderBottom: aba === key ? '2px solid #f59e0b' : '2px solid transparent',
      ...
      {erro && (
        <div style={{ background: '#FEE2E2', color: '#991B1B', padding: '10px 24px', fontSize: 12 }}>
          {erro} — verifique se as variáveis NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY estão configuradas.
        </div>
      )}
      ...
      <div className={'import-panel' + (importOpen ? ' open' : '')}>
          ...
          <div className="preview-wrap" style={{ display: 'block' }}>
            ...
            <div className="preview-scroll">
              <table className="ptbl">
                <thead>
                  <tr>{detectedCols.map((c, i) => <th key={i}>{c}</th>)}</tr>
                </thead>
                <tbody>
                  {previewRows.map((r, ri) => (
                    <tr key={ri}>{detectedCols.map((_, ci) => <td key={ci}>{String(r[ci] || '')}</td>)}</tr>
                  ))}
                </tbody>
              </table>
            </div>
        ...
        <div className="import-footer">
          <div className="import-info">
            {importedRows.length > 0 ? `${importedRows.length} linha(s) encontrada(s) · ${detectedCols.length} coluna(s)` : ''}
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {importedRows.length > 0 && (
              <button className="chip chip-yellow" onClick={doImport} disabled={saving}>
                {saving ? 'Importando...' : '✓ Importar pendências'}
              </button>
            )}
          </div>
        </div>
      ...
      {selecionados.length > 0 && (
        <div style={{ padding: '8px 24px', background: '#1a1a2e', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid rgba(255,255,255,.08)' }}>
          <span style={{ color: 'rgba(255,255,255,.7)', fontSize: 12, fontFamily: "'DM Mono',monospace" }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.id} style={isSel ? { background: 'rgba(220,38,38,.08)' } : {}}>
                      ...
                      <td className="id-cell">
                        ...
                        {p.confianca && p.confianca !== 'alta' && (
                          <span className={'tag-conf ' + p.confianca} title="Confiança da extração automática — revise os campos">{p.confianca}</span>
                        )}
                      </td>
                      ...
                      <td style={{ whiteSpace: 'nowrap', display: 'flex', gap: 3, paddingTop: 8 }}>
      ...
      {aba === 'dashboard' && (
        <div style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
          <div className="metrics-eyebrow" style={{ marginBottom: 16 }}>Resumo financeiro · por loja e responsável</div>

          {/* Cards resumo geral */}
          <div className="metrics" style={{ marginBottom: 28 }}>
            <div className="metric total"><div className="metric-num">∑</div><div className="metric-lbl">Total Pendências</div><div className="metric-val">{metrics.total}</div></div>
            <div className="metric val"><div className="metric-num">R$</div><div className="metric-lbl">Valor Total</div><div className="metric-val sm">{fmt(metrics.val)}</div></div>
            <div className="metric pend"><div className="metric-num">L</div><div className="metric-lbl">Lojas Ativas</div><div className="metric-val yellow">{porLoja.length}</div></div>
            <div className="metric and"><div className="metric-num">R</div><div className="metric-lbl">Responsáveis</div><div className="metric-val">{porResponsavel.length}</div></div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 24, alignItems: 'start' }}>

            {/* ── POR LOJA ── */}
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, padding: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: 'rgba(255,255,255,.5)', marginBottom: 16 }}>
                Valor pendente por loja
              </div>
                ...
                {porLoja.slice(0, 8).map((l) => (
                  <div key={l.loja}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3 }}>
                      <span style={{ color: 'rgba(255,255,255,.8)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 220 }} title={l.loja}>{l.loja}</span>
                      <span style={{ color: '#f59e0b', fontFamily: "'DM Mono',monospace" }}>{fmt(l.valor)}</span>
                    </div>
                    <div style={{ height: 8, background: 'rgba(255,255,255,.06)', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{
                        height: '100%',
                        width: `${maxValorLoja ? (l.valor / maxValorLoja) * 100 : 0}%`,
                ...
                <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,.1)' }}>
                      <th style={{ textAlign: 'left', padding: '6px 8px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Loja</th>
                      <th style={{ textAlign: 'right', padding: '6px 8px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Qtd</th>
                      <th style={{ textAlign: 'right', padding: '6px 8px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Pendente</th>
                      <th style={{ textAlign: 'right', padding: '6px 8px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Atrasada</th>
                      <th style={{ textAlign: 'right', padding: '6px 8px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Valor</th>
                    </tr>
                  </thead>
                  <tbody>
                    {porLoja.map((l) => (
                      <tr key={l.loja} style={{ borderBottom: '1px solid rgba(255,255,255,.04)' }}>
                        <td style={{ padding: '6px 8px', color: 'rgba(255,255,255,.85)', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={l.loja}>{l.loja}</td>
                        <td style={{ padding: '6px 8px', textAlign: 'right', color: 'rgba(255,255,255,.7)' }}>{l.total}</td>
                        <td style={{ padding: '6px 8px', textAlign: 'right', color: '#facc15' }}>{l.pendente}</td>
                        <td style={{ padding: '6px 8px', textAlign: 'right', color: '#f87171' }}>{l.atrasada}</td>
                        <td style={{ padding: '6px 8px', textAlign: 'right', color: '#fff', fontFamily: "'DM Mono',monospace" }}>{fmt(l.valor)}</td>
                      </tr>
                    ))}
                    {porLoja.length === 0 && (
                      <tr><td colSpan={5} style={{ padding: 16, textAlign: 'center', color: 'rgba(255,255,255,.4)' }}>Sem dados ainda</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, padding: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {porResponsavel.slice(0, 10).map((r, idx) => (
                  <div key={r.responsavel} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      width: 22, height: 22, borderRadius: '50%', flexShrink: 0,
                      background: idx === 0 ? '#f59e0b' : idx === 1 ? '#94a3b8' : idx === 2 ? '#b45309' : 'rgba(255,255,255,.1)',
                      color: idx < 3 ? '#000' : 'rgba(255,255,255,.6)',
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3 }}>
                        <span style={{ color: 'rgba(255,255,255,.85)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={r.responsavel}>{r.responsavel}</span>
                        <span style={{ color: 'rgba(255,255,255,.5)', fontFamily: "'DM Mono',monospace", flexShrink: 0, marginLeft: 8 }}>{r.total}</span>
                      </div>
                      <div style={{ height: 6, background: 'rgba(255,255,255,.06)', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{
                          height: '100%',
                          width: `${maxTotalResp ? (r.total / maxTotalResp) * 100 : 0}%`,
                ...
                {porResponsavel.length === 0 && (
                  <div style={{ padding: 16, textAlign: 'center', color: 'rgba(255,255,255,.4)', fontSize: 12 }}>Sem dados ainda</div>
                )}
              </div>

              {/* Mini tabela valor por responsável */}
              <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,.08)' }}>
                <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,.1)' }}>
                      <th style={{ textAlign: 'left', padding: '6px 4px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Responsável</th>
                      <th style={{ textAlign: 'right', padding: '6px 4px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Resolvidas</th>
                      <th style={{ textAlign: 'right', padding: '6px 4px', color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>Valor</th>
                    </tr>
                  </thead>
                  <tbody>
                    {porResponsavel.slice(0, 10).map((r) => (
                      <tr key={r.responsavel} style={{ borderBottom: '1px solid rgba(255,255,255,.04)' }}>
                        <td style={{ padding: '6px 4px', color: 'rgba(255,255,255,.8)', maxWidth: 100, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={r.responsavel}>{r.responsavel}</td>
                        <td style={{ padding: '6px 4px', textAlign: 'right', color: '#4ade80' }}>{r.resolvida}</td>
                        <td style={{ padding: '6px 4px', textAlign: 'right', color: '#fff', fontFamily: "'DM Mono',monospace" }}>{fmt(r.valor)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="modal">
          ...
          <div className="modal-body">
            <div className="form-grid">
              <div className="fg">
                <label>C.Custo / Loja</label>
                <input type="text" placeholder="Ex: RECEITA COMERCIAL LANÇAMENTO" value={form.loja || ''} onChange={(e) => setForm((f) => ({ ...f, loja: e.target.value }))} />
              </div>
              <div className="fg">
                <label>Status</label>
              ...
              <div className="fg">
                <label>Pagador</label>
                <input type="text" placeholder="Nome completo do pagador" value={form.pagador || ''} onChange={(e) => setForm((f) => ({ ...f, pagador: e.target.value }))} />
              </div>
              <div className="fg">
                <label>Empreendimento</label>
                <input type="text" placeholder="Ex: Lottus Residence, SPE Leme do Praia..." value={form.empreendimento || ''} onChange={(e) => setForm((f) => ({ ...f, empreendimento: e.target.value }))} />
              </div>
              <div className="fg">
                <label>Proposta / Contrato</label>
                <input type="text" placeholder="Nº da proposta ou contrato" value={form.proposta || ''} onChange={(e) => setForm((f) => ({ ...f, proposta: e.target.value }))} />
              </div>
              <div className="fg">
                <label>Data Receb.</label>
                <input type="text" placeholder="DD/MM/AAAA" value={form.dataReceb || ''} onChange={(e) => setForm((f) => ({ ...f, dataReceb: e.target.value }))} />
              </div>
              <div className="fg">
                <label>Valor (R$)</label>
                <input type="number" placeholder="0,00" min="0" step="0.01" value={form.valor ?? ''} onChange={(e) => setForm((f) => ({ ...f, valor: e.target.value }))} />
              </div>
              <div className="fg">
                <label>Responsável Atual</label>
                <input type="text" placeholder="Nome do responsável" value={form.responsavel || ''} onChange={(e) => setForm((f) => ({ ...f, responsavel: e.target.value }))} />
              </div>
              <div className="fg full">
                <label>Próxima Ação</label>
                <input type="text" placeholder="Descreva a próxima ação prevista" value={form.acao || ''} onChange={(e) => setForm((f) => ({ ...f, acao: e.target.value }))} />
              </div>
              <div className="fg full">
                <label>Observações</label>
                <textarea
                  placeholder="Digite aqui informações relevantes: tratativas realizadas, datas de contato, documentos pendentes, detalhes do caso..."
                  value={form.obs || ''}
                ...
                <>
                  ...
                  <div className="fg full">
                    <ul className="hist-list">
                      {(form.historico || []).map((h, i) => <li className="hist-item" key={i}>{h}</li>)}
                    </ul>
                    <input
                      className="hist-input"
                      type="text"
                      placeholder="Adicionar novo registro ao histórico..."
                      value={histNew}
                      onChange={(e) => setHistNew(e.target.value)}
                    />
                  </div>
                </>
              )}
          ...
          <div className="modal-foot">
            ...
            <button className="btn btn-primary" onClick={savePendencia} disabled={saving}>
