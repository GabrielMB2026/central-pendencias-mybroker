...
/* ──────────────────────────────────────────────────────────
   Parser do campo "Contrato" do ERP My Broker.
   Texto livre digitado manualmente, sem padrão único.
   Padrão dominante observado:
   "Recebimentos [cfin_NumDocumento] - NOME /PROPOSTA - EMPREENDIMENTO - Unidade X Bl/Qd Y - VENDA NA ABA: STATUS"
   Tenta extrair o máximo possível e atribui confiança (alta/media/baixa).
   O texto original é SEMPRE preservado em Observações.
   ────────────────────────────────────────────────────────── */
...
function nextId(existing) {
  // Gera o próximo ID PND-XXX baseado no maior já existente
  let max = 0;
  existing.forEach((p) => {
...
export default function Home() {
  ...
  function exportExcel() {
    ...
    ws['!cols'] = [
      ...
      { wch: 12 }, { wch: 14 }, { wch: 14 }, { wch: 14 }, { wch: 20 }, { wch: 32 }, { wch: 42 }, { wch: 60 },
    ];
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Proposta</th><th>Data Receb.</th><th>Valor</th><th>Tipo</th>
                ...
                filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td className="tipo-cell">{p.tipo}</td>
        ...
        <div className="modal">
          ...
          <div className="modal-body">
            <div className="form-grid">
              ...
              <div className="fg">
                <label>Tipo</label>
                <select value={form.tipo || 'Documentação'} onChange={(e) => setForm((f) => ({ ...f, tipo: e.target.value }))}>
                  <option>Documentação</option><option>Pagamento</option><option>Assinatura</option><option>Vistoria</option><option>Outro</option>
                </select>
              </div>
              <div className="fg">
